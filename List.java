...
public class List {
    ...
    public void insert(Object next, int index) {
        // Else if's so we only hit one statement
        // no possible way for the index to be less than 0, so check for this first and throw error if it is the case
        if(index < 0) {
        ...
        // first checking if the list only has one element and if the index is greater than 1, since that wouldn't be allowed
        else if (head.next == null && index > 1) {
            System.out.println("Throw exception temp holder");
        }
        // valid case of if there is one element and the desired index is 0, we replace the head with next and have it point to the old head
        else if (head.next == null && index == 0 ) {
            Node prev = head;
            Node current = new Node (next, prev);
            head = current;
        }
        // Valid case of if there is one element and the desired index is 1, we insert it after the head
        else if (head.next == null && index == 1) {
